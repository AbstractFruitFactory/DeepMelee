...
class MenuManagerTest(unittest.TestCase):
    def setUp(self):
        ...
        self.state.players = []
        for _ in range(4):
            self.state.players.append(state.State())

    def test_menu_manager_press_start_even(self):
        self.state.frame = 0
        self.mm.press_start_lots(self.state, self.pad)
        self.pad.press_button.assert_called_with(pad.Button.START)

    def test_menu_manager_press_start_odd(self):
        self.state.frame = 1
        self.mm.press_start_lots(self.state, self.pad)
        self.pad.release_button.assert_called_with(pad.Button.START)
    ...
    def test_menu_manager_pick_fox_presses(self):
        ...
        self.pad.press_button.assert_called_with(pad.Button.A)
